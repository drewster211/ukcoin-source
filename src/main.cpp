...
#if defined(NDEBUG)
# error "UKCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2f7ea72c8c863162d366c52860bb8ed6a1d118eb9adc9f72e3e94ee548b82521");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UKCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UKCoin Signed Message:\n";
...
{
    // UKCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 90 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // UKCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // UKCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1457189645, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "George Osborne drops plans to cut tax relief";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b6a57e0e609cdaebc5a315978c3a3b20a9d526b8991ac05208e8493b1987ccb5b962472fab1a219e82a1d61a528afa34f953b760d0f26db825282357fe14c0de") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1457189645;
        ...
        assert(block.hashMerkleRoot == uint256("0x7cbfbb6c187f5a8cd2e760418661b5173908e1da1d970a2e1d345049613e8a01"));
...
unsigned char pchMessageStart[4] = { 0xc2, 0x80, 0xe5, 0x73 }; // UKCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UKCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UKCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UKCoinMiner(CWallet *pwallet)
{
    printf("UKCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ukcoin-miner");
    ...
    try { loop {
        ...
        printf("Running UKCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UKCoinMiner, pwallet));
}
